...
class HeadlessDisplayer(object):
    ...
    def finished(self):
        ...
        # SDSC
        #
        # we need this script to terminate after the download completes.
        # downloaded RPMs will be served by bylaunchmany which is started
        # by anaconda
        #
        self.doneflag.set()
    ...
    def display(self, statistics):
        ...
        # SDSC
	#
	# if there are no seeds and no peers, then quit
	#
	# rocks-bt.py calls this script -- if it quits without getting the
	# file, then rocks-bt.py will use 'wget' to get the file
        #
        if statistics.has_key('numSeeds') and statistics['numSeeds'] == 0 and \
		statistics.has_key('numPeers') and statistics['numPeers'] == 0:
            self.doneflag.set()
...
if __name__ == '__main__':
    ...
    #
    # if we made it here, then there was an error
    #
    sys.exit(-1)
