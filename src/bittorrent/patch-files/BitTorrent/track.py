...
class Tracker(object):
    ...
    def add_data(self, infohash, event, ip, paramslist):
        ...
        if event not in \
		['started', 'completed', 'stopped', 'snooped', 'done', None]:
            raise ValueError, 'invalid event'
        #
        # SDSC
        #
    ...
    def connectback_result(self, result, downloadid, peerid, ip, port):
        ...
        # 
        # fake out the tracker into thinking that a 'seeder' is operational
        # at the other end of the connection. this allows us to send messages
	# to tracker to let it know which files at the other end of the
	# connection are available (and not available).
        # 
        result = 1
    ...
    #
    # this function removes all references to any file that is being tracked
    # it is called when a node is done installing, therefore, it will not
    # be participating as a potential downloader
    #
    def done_peer(self, myid):
	for key, peers in self.downloads.items():
		if myid in peers:
			self.delete_peer(key, myid)
	return
    #
    # SDSC
    #
