 ...
 *
 * $Id: addr.c,v 1.1 2008/01/04 23:04:43 bruno Exp $
 */
...
addr_pton(const char *src, struct addr *dst)
{
	...
	for (i = 0; i < sizeof(tmp) - 1; i++) {
		if (src[i] == '/') {
			...
			mask = &src[i+1];
			if (strchr(mask, '.')) {
				/* The mask is specified like /255.0.0.0 */
				if (ip_pton(mask, &addr_mask.addr_ip) != 0)
					goto error;

				addr_mtob(&addr_mask.addr_ip, 
					sizeof(addr_mask.addr_ip), (uint16_t*) &bits);
			} else {
				/* The mask is specified like /8  */
				bits = strtol(mask, &ep, 10);
