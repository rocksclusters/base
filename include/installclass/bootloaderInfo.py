...
#            f.write('\tmakeactive\n')
        ...
        if not justConfigFile:
            ...
            for cmd in cmds:
                ...
                rhpl.executil.execWithRedirect('/sbin/grub' ,
                                    [ "grub",  "--batch", "--no-floppy",
                                      "--device-map=/boot/grub/device.map" ],
                                    stdin = p[0],
                                    stdout = "/tmp/grub-stdout.log", stderr = "/tmp/grub-stderr.log",
                                    root = instRoot)
                os.system('md5sum /mnt/sysimage/boot/grub/stage2 >> /tmp/grub-install.log')
                # ROCKS
                os.close(p[0])
            ...
            # ROCKS
            f.close()
            c = "chroot /mnt/sysimage /sbin/grub-install '%s' " % stage1Target
            c += '>> /tmp/grub-install.log 2>&1'
            os.system(c)
