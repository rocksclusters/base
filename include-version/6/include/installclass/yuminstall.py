...
class AnacondaYumRepo(YumRepository):
    ...
    def needsNetwork(self):
        ...
        # ROCKS
	# If a frontend, can build without network and use local DVD rolls
...
class AnacondaYum(YumSorter):
    ...
    def _getTreeinfo(self, baseurl, proxy_url, sslverify):
        ...
        """
	# ROCKS 
	# We don't generate treeinfo's, so this would just time out anyway 
	return None
	# ROCKS
	...
	# XXX This max value should be a runtime tunable
	if repo.timeout < 120.0:
            repo.timeout = 120.0
        else:
            repo.timeout += 30.0

        log.info("ROCKS:timeout: %s" % repo.timeout)
